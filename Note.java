...
public class Note implements Comparable<Note> {
    ...
    private double length; // holds the length the user input
     ...
     * @param length    this is the double value the user entered for the length of the note
    ...
    //This was my first attempt at getting the variables sub-sorted with the toCompare method, however it's a bit
    //long winded.

//    public int frequencyCompare(Note compareNote){
//        if(this.computeFrequency() < compareNote.computeFrequency()){
//            return -1;
//        }
//        else if(this.computeFrequency() == compareNote.computeFrequency()){
//            return 0;
//        }
//        else{
//            return 1;
...
//        if(this.getLength() < compareNote.getLength()){
//            return -1;
//        }
//        else if(this.getLength() == compareNote.getLength()){
//                return  frequencyCompare(compareNote);
//            }
//            else{
//                return 1;
//            }
//        }
    ...
    //I copied Kevin's version which is a similar idea to where my code above was going, but his is much more precise
    //in that it checks to see if it even needs to sort by frequency by seeing if the lengths are equal or not.

    /**
     * This method compares the length and frequency of a Note object to another Note object.
     * @param compareNote Note
     * @return int
     */
    @Override
    public int compareTo(Note compareNote){

        int result = Double.compare(this. length, compareNote.length);

        if(result == 0) {
            return Double.compare(this.computeFrequency(), compareNote.computeFrequency());
        }
        else{
            return result;
        }
