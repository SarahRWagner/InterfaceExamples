...
public class Quadratic implements Comparable <Quadratic>{
     ...
     */
    public Complex[] findComplexRoots() {

        Complex[] complexRoots = new Complex[2]; // creating the Complex array
        ...
        Complex complex1 = new Complex(realPart, (Math.sqrt(-discriminant) / (2 * a))); // the first root
        Complex complex2 = new Complex(realPart, (-(Math.sqrt(-discriminant) / (2 * a)))); // the second root

        complexRoots[0] = complex1; // add the two roots into an array, so that both can be
        complexRoots[1] = complex2; // passed back to main

        return complexRoots;
    }


    public Complex[] findRealRoots() { // if the method above finds the roots are real...

        Complex[] realRoots = new Complex[2];
        ...
        realRoots[0] = real1; // add the two roots into the array
        realRoots[1] = real2;

        return realRoots; // return the array back to main

    }

    public int compareTo(Quadratic compareQuad){

        if (this.getA() < compareQuad.getA()) {
            return -1;
        } else if (this.getA() == compareQuad.getA()) {
            return 0;
        } else {
            return 1;
        }
